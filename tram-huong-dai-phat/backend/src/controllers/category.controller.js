...
const createCategory = async (req, res) => {
  try {
    ...
    const newCategory = await categoryRepository.createCategory({ name, description, image_url });
...
const updateCategory = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, image_url } = req.body;
    const updatedCategory = await categoryRepository.updateCategory(id, { name, description, image_url });
    if (!updatedCategory) {
      return res.status(404).json({ message: "Không tìm thấy danh mục" });
    }
    res.status(200).json({ message: "Cập nhật danh mục thành công", data: updatedCategory });
  } catch (error) {
    console.error("Lỗi updateCategory:", error.message);
